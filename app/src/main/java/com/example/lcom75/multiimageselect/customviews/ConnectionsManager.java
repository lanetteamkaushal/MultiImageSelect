...
public class ConnectionsManager {
    ...
    public int sendRequest(final TLObject object, final RequestDelegate onComplete, final QuickAckDelegate onQuickAck, final int flags, final int datacenterId, final int connetionType, final boolean immediate) {
        ...
        Utilities.stageQueue.postRunnable(new Runnable() {
            @Override
            public void run() {
                Log.d("tmessages", "send request " + object + " with token = " + requestToken);
    ...
    public void setAppPaused(final boolean value, final boolean byScreenState) {
        if (!byScreenState) {
            appPaused = value;
            Log.d("tmessages", "app paused = " + value);
        }
        if (value) {
            if (lastPauseTime == 0) {
            ...
            Log.e("tmessages", "reset app pause time");
    ...
    public static void onUnparsedMessageReceived(int address) {
        try {
            ...
            if (message instanceof TLRPC.Updates) {
                Log.d("tmessages", "java received " + message);
                AndroidUtilities.runOnUIThread(new Runnable() {
                    @Override
                    public void run() {
                        if (getInstance().wakeLock.isHeld()) {
                            Log.d("tmessages", "release wakelock");
        ...
        AndroidUtilities.runOnUIThread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    Log.d("tmessages", "acquire wakelock");
                } catch (Exception e) {
                    Log.e("tmessages", e.getMessage());
                }
            }
